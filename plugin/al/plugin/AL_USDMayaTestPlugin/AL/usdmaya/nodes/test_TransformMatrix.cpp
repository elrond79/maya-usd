...
TEST(Transform, splitPivot)
{
  ...
  auto constructTransformChain = [&] ()
  {
    ...
    EXPECT_EQ(matchingXforms.size(), 4u);
    a.SetXformOpOrder(ops);
...

  {
    ...
    const GfVec3f scaleVal(1.0, 0.5, 1.0);
    ...
    // ...and make sure this is reflected correctly in xformOps
    {
      ...
      EXPECT_TRUE(xformOps[6].GetAs(&vec3fVal, UsdTimeCode::Default()));
      EXPECT_EQ(scaleVal, vec3fVal);
